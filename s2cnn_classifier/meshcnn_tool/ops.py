...
from meshcnn_tool import utils
...
class _MeshConv(nn.Module):
    def __init__(self, in_channels, out_channels, mesh_file, stride=1, bias=True):
        ...
        G = utils.sparse2tensor(pkl['G'])  # gradient matrix V->F, 3#F x #V
...
class MeshConv(_MeshConv):
    def __init__(self, in_channels, out_channels, mesh_file, stride=1, bias=True):
        ...
        if stride == 2:
            ...
            L = utils.sparse2tensor(pkl['L'].tocsr()[:self.nv_prev].tocoo()) # laplacian matrix V->V
            F2V = utils.sparse2tensor(pkl['F2V'].tocsr()[:self.nv_prev].tocoo())  # F->V, #V x #F
        else: # stride == 1
            self.nv_prev = pkl['V'].shape[0]
            L = utils.sparse2tensor(pkl['L'].tocoo())
            F2V = utils.sparse2tensor(pkl['F2V'].tocoo())
        self.register_buffer("L", L)
        self.register_buffer("F2V", F2V)

    def forward(self, input):
        # compute gradient
        grad_face = utils.spmatmul(input, self.G)
        grad_face = grad_face.view(*(input.size()[:2]), 3, -1).permute(0, 1, 3, 2) # gradient, 3 component per face
        laplacian = utils.spmatmul(input, self.L)
        ...
        grad_vert_ew = utils.spmatmul(grad_face_ew, self.F2V)
        grad_vert_ns = utils.spmatmul(grad_face_ns, self.F2V)
...
class MeshConv_transpose(_MeshConv):
    def __init__(self, in_channels, out_channels, mesh_file, stride=2, bias=True):
        ...
        L = utils.sparse2tensor(pkl['L'].tocoo()) # laplacian matrix V->V
        F2V = utils.sparse2tensor(pkl['F2V'].tocoo()) # F->V, #V x #F
        self.register_buffer("L", L)
        self.register_buffer("F2V", F2V)
