...
from s2cnn import s2_near_identity_grid, S2Convolution, so3_near_identity_grid, SO3Convolution, so3_integrate
...
class Model(nn.Module):
    def __init__(self, nclasses):
        ...
        # S2 layer
        grid = s2_near_identity_grid(max_beta=0, n_alpha=2 * self.bandwidths[0], n_beta=1)
        ...
        for l in range(1, len(self.features) - 2):
            ...
            grid = so3_near_identity_grid(max_beta=0, max_gamma=0, n_alpha=2 * b_in, n_beta=1, n_gamma=1)
